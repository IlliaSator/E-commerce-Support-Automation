...
Import workflows from `n8n/workflows/`. For portfolio review, import `techgear_automation_canvas_v2_workflow.json`; it is the large connected MVP canvas. It includes real n8n AI cluster nodes: AI Agent, Chat Model placeholder, Postgres Chat Memory, backend HTTP Request Tools, Code Tool, Think Tool, Calculator Tool, and controlled ticket tools. Replace placeholder credential names in the n8n UI when enabling real Telegram, Google Sheets, Supabase, PostgreSQL memory, or optional LLM integrations. Backend webhook env vars point to Docker network URLs such as `http://n8n:5678/webhook/support-event-hub`.

## Configured n8n MVP Canvas v2

The screenshot below shows the configured n8n editor canvas for the MVP. Backend webhooks, AI decision routing, AI Agent drafting with an LLM model, Postgres memory, backend tools, code/risk checks, policy thinking, SLA calculation, controlled ticket tools, PostgreSQL audit snapshots, SLA jobs, feedback handling, Telegram manager alerts, Google Sheets logging, Supabase CRM mirror branches, and error handling are connected in one support automation workflow.
